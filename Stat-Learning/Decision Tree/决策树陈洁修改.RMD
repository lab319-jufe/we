...
```{r warning=FALSE,message=FALSE}
plot(fit1)
text(fit1,pretty=0,cex=0)#pretty=0用于输出所有定性预测变量的类别名，而非首字母
```
...
fit3
```
**结果解读**:"size"是所考虑的每棵树的终端结点数，"dev"是对应的交叉验证错误率，当size为8时，交叉验证错误率最低，共有75 个交叉验证误差， "k" 是使用的成本复杂性参数(代价复杂性剪枝中的$\alpha$,可以理解为惩罚系数)
